...
func requireSplicedInOrder(t *testing.T, fake *recordingSpliceChunksClient, want []*repb.Digest, rn *digest.CASResourceName) {
	...
	require.True(t, digest.Equal(rn.GetDigest(), requests[0].GetBlobDigest()))
	require.Equal(t, rn.GetInstanceName(), requests[0].GetInstanceName())
	require.Equal(t, repb.DigestFunction_SHA256, requests[0].GetDigestFunction())
	require.Equal(t, repb.ChunkingFunction_FAST_CDC_2020, requests[0].GetChunkingFunction())
	var sent []*repb.Digest
	for i, req := range requests {
		if i > 0 {
			require.Nil(t, req.GetBlobDigest(), "metadata must only be set on the first request")
...
func TestStreamedSplicerSendsChunksInOrder(t *testing.T) {
	...
	// Availability arrives out of order; chunks must still be spliced in
	// addChunk order.
...
func TestStreamedSplicerRepeatsDuplicateChunks(t *testing.T) {
	...
	// The same chunk can occur at multiple positions within one blob; a single
	// availability report must unblock every position.
...
func TestStreamedSplicerBatchesLargeDigestLists(t *testing.T) {
	...
	require.True(t, spliced)
	requireSplicedInOrder(t, fake, digests, rn)
	require.GreaterOrEqual(t, len(fake.recordedRequests()), 2)
}
...
func TestStreamedSplicerSurfacesFatalErrorsAtCommit(t *testing.T) {
	...
	// Availability triggers the failing Send. Subsequent calls may observe the
	// terminal error or not depending on timing, so only commit's result is
	// asserted.
...
func TestStreamedSplicerAbsorbsFallbackErrors(t *testing.T) {
	...
	require.NoError(t, splicer.addChunk(digests[0]))
	require.NoError(t, splicer.markRemoteAvailable(digests))
	// Wait for the failed Send to terminate the splicer, then check that
	// callers keep seeing nil errors for fallback-eligible failures.
	<-splicer.done
	require.NoError(t, splicer.addChunk(digests[1]))
	require.NoError(t, splicer.markRemoteAvailable(digests))
