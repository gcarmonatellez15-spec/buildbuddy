...
func (p *CacheProxy) SpliceBlob(ctx context.Context, req *repb.SpliceBlobRequest) (*repb.SpliceBlobResponse, error) {
...
func (p *CacheProxy) SpliceChunks(stream repb.ContentAddressableStorage_SpliceChunksServer) error {
	...
	for {
		...
		if err := clientStream.Send(req); err != nil {
			if err == io.EOF {
				// The remote ended the stream early; surface its real status.
				return finish()
			}
			return err
		}
